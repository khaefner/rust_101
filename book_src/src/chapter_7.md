...
0x8180 is a hexadecimal representation of a 16-bit integer. In binary, it's 1000000110000000.
    Let's break down the meaning of these bits in the context of a DNS response:
        The first bit (most significant) being 1 indicates that this is a response.
        The next four bits (0000) represent the Opcode, which is 0 for a standard query.
        The next bit (0) indicates that the server is not authoritative for the requested domain in this simplified example.
        The T bit (Truncation) is 0, meaning the message is not truncated.
        The R bit (Recursion Desired) in the query is usually copied to the response. Here, the second to last bit is 1, indicating that recursion is available on this server (though this simple implementation doesn't actually perform recursion).
        The last three bits (000) represent the Rcode (Response code). 0 means no error.

3. parse_query_domain Function:
Rust
...
4. create_response_packet Function:
Rust
